...
# ///
"""ゴール条件付き ACT の sim 内評価ジョブ（並列B）。

事前宣言ゴール方式（relabeling 禁止 — docs/HANDOFF.md 設計）:
- ゴールは datagen と同一の円盤（固定中心 = 到達点定数 + (0,+0.022)、半径 0.018）から
  エピソード毎に事前サンプルし、observation.environment_state として毎ステップ入力
- 成功 = 終端でトマトが宣言点から THRESH（既定 2.5cm）以内 かつ 皿ゾーン内・静定
- distance-to-goal の分布も stats.json に保存（成功率より情報量が多い）

実行:
    hf jobs uv run --flavor t4-small --timeout 2h --secrets HF_TOKEN \
        -v hf://buckets/YUGOROU/gyoza-sim:/gyoza --env GYOZA_DATA=/gyoza \
        --env POLICY=YUGOROU/act_gyoza_pickplace_goal_synth \
        --env RUN=eval_act_goal --env EPISODES=50 --env SEED=42 \
        jobs/act_eval_goal_job.py

env vars: POLICY / RUN / EPISODES / VIDEO_N(5) / SEED / MAX_STEPS(450) / THRESH(0.025)
          / DISK_CX(0.204) / DISK_CY(-0.096) / OFFSET_R(0.018)
"""
...
def main():
    ...
    run = os.environ.get("RUN", "eval_act_goal")
    ...
    # datagen のゴール円盤と同一定義（到達点定数 (0.204,-0.118) + (0,+0.022)）
    ...
    # 重要: 正規化はモデル外の processor パイプライン（act_eval_job.py の同コメント参照）
    ...
    print(f"[eval-goal] policy={policy_repo} episodes={episodes} thresh={thresh}", flush=True)
    ...
    for ep in range(episodes):
        ...
        vel = float(np.linalg.norm(env.data.cvel[env._tomato_body]))
    ...
    summary = dict(policy=policy_repo, episodes=episodes, seed=seed, thresh=thresh,
    ...
    print("[eval-goal] DONE", json.dumps({k: v for k, v in summary.items() if k != "per_episode"}), flush=True)
